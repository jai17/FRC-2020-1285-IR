...
import frc.team254.lib.trajectory.TrajectorySamplePoint;
...
public class DriveMotionPlanner implements CSVWritable {
    ...
    public Trajectory<TimedState<Pose2dWithCurvature>> generateTrajectory(boolean reversed,
        ...
        timed_trajectory.setDefaultVelocity(default_vel / NumberConstants.kSwerveMaxSpeedInchesPerSecond);
    ...
    protected Translation2d updatePurePursuit(Pose2d current_state) {
        double lookahead_time = NumberConstants.kPathLookaheadTime;
        ...
        while (actual_lookahead_distance < NumberConstants.kPathMinLookaheadDistance
        ...
        if (actual_lookahead_distance < NumberConstants.kPathMinLookaheadDistance) {
                    ...
                    new Pose2dWithCurvature(
                            lookahead_state.state().getPose()
                                    .transformBy(Pose2d.fromTranslation(new Translation2d((mIsReversed ? -1.0 : 1.0)
                                            * (NumberConstants.kPathMinLookaheadDistance - actual_lookahead_distance),
                                            0.0))),
                            0.0),
        ...
        SmartDashboard.putNumber("Path Velocity",
                lookahead_state.velocity() / NumberConstants.kSwerveMaxSpeedInchesPerSecond);
        ...
        double normalizedSpeed = Math.abs(mSetpoint.velocity()) / NumberConstants.kSwerveMaxSpeedInchesPerSecond;
